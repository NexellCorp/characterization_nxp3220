			...
			struct json_object *Jobj, const char *Key)
{
	...
	if (!json_object_is_type(obj, json_type_int)) {
		LogE("%s: key[%s] not int :%s\n",
			m_File, Key, json_object_to_json_string(obj));
		return 0;
	}

	return (int)json_object_get_int(obj);
}

bool TaskParser::GetKeyValueBool(
			struct json_object *Jobj, const char *Key)
{
	assert(m_pJobj && Jobj);
...
int TaskParser::GetKeyValueArray(
			struct json_object *Jobj, const char *Key, int Index)
{
	...
	if (Index > length - 1) {
		LogE("%s: key[%s] %d over arrays %d : %s\n",
